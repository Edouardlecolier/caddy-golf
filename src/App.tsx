...
function Sparkline({data, color, h=36, invert=false}){
  ...
  const mn=Math.min(...plotData), mx=Math.max(...plotData), rng=mx-mn||1;
  const W=180, H=h;
  const pts=plotData.map((v,i)=>`${(i/(plotData.length-1))*W},${H-((v-mn)/rng)*(H-6)+3}`).join(" ");
  return (
    <svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",height:h,display:"block"}}>
      <polyline points={pts} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      {plotData.map((v,i)=>(
        <circle key={i} cx={(i/(plotData.length-1))*W} cy={H-((v-mn)/rng)*(H-6)+3} r="3" fill={color} stroke={WHITE} strokeWidth="1.5"/>
      ))}
    </svg>
  );
}
...
// about what this is.
// ═══════════════════════════════════════════════════════════════════════════
function FinisherGame({game, onSave, onExit, lang, accountName, units}){
  ...
  if(mode==="playing") return (
    <div>
      ...
      <div style={{background:WHITE,borderRadius:18,padding:18,marginBottom:14,border:"1px solid "+BORDER,boxShadow:"0 2px 12px rgba(0,0,0,0.05)"}}>
        ...
        <p style={{color:TEXT_S,fontSize:13,lineHeight:1.6,margin:"0 0 10px"}}>{cvt(station.desc,units)}</p>
        {station.why&&(
          <div style={{background:OFF,borderRadius:12,padding:"10px 14px",marginBottom:10,border:"1px solid "+BORDER}}>
            ...
            <div style={{color:TEXT_S,fontSize:12,lineHeight:1.6}}>{cvt(station.why,units)}</div>
          </div>
        )}
...
function TrainTab({sessions, onSave, settings, userId, accountName}){
  ...
  if(step==="finisherPlay"&&activeFinisher) return (
    <FinisherGame game={activeFinisher} lang={lang} accountName={accountName} units={units}
      onSave={(s)=>onSave(s)}
      ...
      <div>
        ...
        <div style={{background:WHITE,borderRadius:18,padding:18,marginBottom:14,border:"1px solid "+BORDER,boxShadow:"0 2px 12px rgba(0,0,0,0.05)"}}>
          ...
          {guide.why&&(
            <div style={{background:OFF,borderRadius:12,padding:"10px 14px",marginBottom:10,border:"1px solid "+BORDER}}>
              <div style={{color:SLATE,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>{t(lang,"howWhy")}</div>
              <div style={{color:TEXT_S,fontSize:12,lineHeight:1.6}}>{cvt(guide.why,units)}</div>
            </div>
          )}
