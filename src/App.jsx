...
const HANDICAP_BENCHMARKS = [
  ...
  {hcp:20, putts:36.1, girPct:22.4, firPct:42.8, updownPct:21.7, threePuttsPerRound:3.3},
...
// approach play (GIR), not off the tee (FIR). Returns null for any handicap
...
function deriveHandicaps(roundCount, avgPutts, threePuttAvg18, updownAvgPct, girPct, firPct){
  if(roundCount < 3) return {puttingHcp:null, shortGameHcp:null, longGameHcp:null};
...
function computeGoalMetric(metricKey, sessions){
  ...
  if(rounds.length<3) return null; // not enough data yet for any round-based metric
  ...
  const girPct = allH.length ? (allH.filter(h=>h.gir).length/allH.length)*100 : null;
  const firHoles = allH.filter(h=>h.par>3);
  const firPct = firHoles.length ? (firHoles.filter(h=>h.fir).length/firHoles.length)*100 : null;
  const updownAttempts = allH.filter(h=>h.gir===false);
...
function computeSingleRoundMetric(metricKey, round){
  ...
  if(metricKey==="girPct") return Math.round((holes.filter(h=>h.gir).length/holes.length)*1000)/10;
  if(metricKey==="firPct"){
  ...
  if(metricKey==="updownPct"){
    const attempts = holes.filter(h=>h.gir===false);
    return attempts.length ? Math.round((attempts.filter(h=>(h.putts||0)<=1).length/attempts.length)*1000)/10 : null;
  }
  return null;
}
...
function TrainTab({sessions, onSave, settings, userId, accountName, hand}){
  ...
  if(step==="focus") return (
    <div>
      ...
      {(()=>{
        // One banner per active goal that maps to a specific exercise — not
        // just the first match, and not just a focus area. Goals like
        // overall Handicap or Score vs Par don't map to one exercise, so
        // they're skipped here even if active.
        ...
        for(const g of goals){
          ...
          const exFocusId = metric.focusId;
          banners.push(
            <button key={g.metricKey} onClick={()=>{
                const f=FOCUS_AREAS.find(fa=>fa.id===exFocusId);
                if(f){ setFocus(f); setList(EXERCISES[f.id]); }
                setEx(exercise); setStep("active");
              }}
              style={{width:"100%",background:PURPLE_GRAD,border:"none",borderRadius:16,padding:"14px 16px",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:12,marginBottom:12,boxShadow:"0 3px 14px rgba(124,92,191,0.3)"}}>
              <div style={{width:40,height:40,background:"rgba(255,255,255,0.18)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:19}}>{exercise.emoji}</div>
              <div style={{flex:1}}>
                <div style={{color:"rgba(255,255,255,0.7)",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1}}>{t(lang,"recommendedFromGoal")}: {t(lang,"goalMetric_"+g.metricKey)}</div>
                <div style={{color:WHITE,fontWeight:700,fontSize:14}}>{tx(lang,exercise.id,"name",exercise.name)}</div>
                <div style={{color:"rgba(255,255,255,0.65)",fontSize:11,marginTop:1}}>{hasData?Math.round(current*10)/10+(metric.suffix||""):"—"} → {g.target}{metric.suffix||""}</div>
              </div>
              <div style={{color:WHITE,fontSize:13,fontWeight:700}}>→</div>
            </button>
          );
        }
        return banners.length>0 ? <div style={{marginBottom:8}}>{banners}</div> : null;
      })()}
...
// ═══════════════════════════════════════════════════════════════════════════
function DetailedTrendChart({points, color, invert, suffix, lang}){
  ...
  // Every point gets its own legible label, no matter how many there are.
  const MIN_PER_POINT = 30; // px of horizontal room per point, enough for a 2-3 digit label with breathing room
  ...
  const plotValues = invert ? rawValues.map(v=>-v) : rawValues;
  const vMin = Math.min(...plotValues);
  const vMax = Math.max(...plotValues);
      ...
      <div style={{overflowX:needsScroll?"auto":"visible",WebkitOverflowScrolling:"touch"}}>
        <svg viewBox={"0 0 "+W+" "+H} style={needsScroll ? {width:W,height:H,display:"block"} : {width:"100%",height:"auto",display:"block"}}>
          {[0,0.25,0.5,0.75,1].map(f=>{
            const realVal = invert ? -(vMax-f*span) : (vMax-f*span);
            return (
              <g key={f}>
          ...
          <polyline points={plotValues.map((v,i)=>xAt(i)+","+yAt(v)).join(" ")} fill="none" stroke={color} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
          {plotValues.map((v,i)=>{
            const isEdge = i===0||i===plotValues.length-1;
            return <circle key={i} cx={xAt(i)} cy={yAt(v)} r={isEdge?4:2.5} fill={color} stroke={WHITE} strokeWidth="1.2"/>;
          })}
          {/* Every point gets its own value label now — safe to do since
              the chart has real per-point width to work with (no more
              cramming 15 labels into a fixed 320px that would overlap). */}
          {plotValues.map((v,i)=>{
            const yPos = yAt(v);
            const labelAbove = yPos > padT+14;
            return (
              <text key={"lbl"+i} x={xAt(i)} y={labelAbove?yPos-9:yPos+15} fontSize="9" fill={color} fontWeight="700" textAnchor="middle">
                {rawValues[i]}{suffix||""}
              </text>
            );
          })}
          {points.map((p,i)=>(
            <text key={i} x={xAt(i)} y={H-6} fontSize="8" fill={SLATE_L} textAnchor="middle">{fmtChartAxis(p.ts)}</text>
          ))}
        </svg>
      </div>
    </div>
...
function ScorecardHistoryDetail({rounds, lang, onBack}){
  ...
  const perRoundStatsPts = ranged.map(r=>{
    ...
    return {
      ...
      girPct: holeCount ? (girCount/holeCount)*100 : null,
      firPct: firHoles.length ? (firCount/firHoles.length)*100 : null,
    };
  });
  ...
  const updownTrendPts = perRoundStatsPts.map(r=>({ts:r.ts, value:Math.round(r.updowns18*10)/10}));
  ...
  const CHART_METRICS = {
    score: {points:scoreTrendPts, color:NAVY, invert:true, suffix:"", labelKey:"scoreVsPar", helpKey:"upMeansImproving"},
    threePutts: {points:threePuttTrendPts, color:RED, invert:true, suffix:"", labelKey:"threePutts", helpKey:"downMeansImproving"},
    updown: {points:updownTrendPts, color:GREEN, invert:false, suffix:"", labelKey:"upAndDown", helpKey:"upMeansImproving"},
    avgPutts: {points:avgPuttsTrendPts, color:PURPLE, invert:true, suffix:"", labelKey:"avgPutts", helpKey:"downMeansImproving"},
    gir: {points:girTrendPts, color:GREEN, invert:false, suffix:"%", labelKey:null, helpKey:"upMeansImproving"},
    fir: {points:firTrendPts, color:GREEN, invert:false, suffix:"%", labelKey:null, helpKey:"upMeansImproving"},
  };
        ...
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:18}}>
            {[
              ...
              [t(lang,"upAndDown"),avgUpdown??"—",null],
          ...
          {sampledPoints.length>1 ? (
            <div style={{marginBottom:18}}>
              ...
              <DetailedTrendChart points={sampledPoints} color={activeMetric.color} invert={activeMetric.invert} suffix={activeMetric.suffix} lang={lang}/>
            </div>
          ) : (
...
function HandicapHistoryDetail({rounds, lang, onBack}){
  ...
  const snapshots = roundsChrono.map((r,i)=>{
    ...
    const updownSuccess = updownAttempts.filter(h=>(h.putts||0)<=1);
    const updownPct = updownAttempts.length ? (updownSuccess.length/updownAttempts.length)*100 : null;
    const girPct = holes.length ? (holes.filter(h=>h.gir).length/holes.length)*100 : null;
    const firHoles = holes.filter(h=>h.par>3);
    const firPct = firHoles.length ? (firHoles.filter(h=>h.fir).length/firHoles.length)*100 : null;
  ...
  const rangedGirPct = rangedHoles.length ? (rangedHoles.filter(h=>h.gir).length/rangedHoles.length)*100 : null;
  const rangedFirHoles = rangedHoles.filter(h=>h.par>3);
  const rangedFirPct = rangedFirHoles.length ? (rangedFirHoles.filter(h=>h.fir).length/rangedFirHoles.length)*100 : null;
  const latest = {
  ...
  const HANDICAP_CHART_METRICS = {
    putting: {points:puttingTrendPts, labelKey:"puttingHcp"},
    shortGame: {points:shortGameTrendPts, labelKey:"shortGameHcp"},
    longGame: {points:longGameTrendPts, labelKey:"longGameHcp"},
  };
        ...
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:18}}>
            {[[t(lang,"puttingHcp"),latest?.puttingHcp],[t(lang,"shortGameHcp"),latest?.shortGameHcp],[t(lang,"longGameHcp"),latest?.longGameHcp]].map(([l,v])=>(
              <div key={l} style={{background:PURPLE_BG,borderRadius:14,padding:"10px 6px",textAlign:"center",border:"1px solid "+PURPLE+"33"}}>
                ...
                <div style={{color:NAVY,fontSize:17,fontWeight:700,fontFamily:"Georgia,serif"}}>{fmtDerivedHcp(v)}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:18}}>
            {[...ranged].sort((a,b)=>b.ts-a.ts).map(r=>{
              ...
              const girPctRound = holes.length ? Math.round((holes.filter(h=>h.gir).length/holes.length)*100) : null;
              const firHolesRound = holes.filter(h=>h.par>3);
              const firPctRound = firHolesRound.length ? Math.round((firHolesRound.filter(h=>h.fir).length/firHolesRound.length)*100) : null;
              return (
...
function AddGoalModal({sessions, hdcp, lang, existingMetricKeys, onSave, onClose}){
  ...
  function validateTarget(num, metric){
    ...
    if(num<0) return t(lang,"targetCannotBeNegative");
...
function computeGoalTrend(metricKey, sessions, createdAt){
  ...
  // months ago before you even set the target. The very first point still
  // needs a 3-round window to compute a real value, so the chart genuinely
  // starts blank until 3 rounds have been logged since goal creation.
...
function RaiseBarModal({goal, lang, onConfirm, onClose}){
  ...
  let suggested = metric.lowerIsBetter ? Math.max(0, goal.target-originalGap*0.5) : goal.target+originalGap*0.5;
...
function GoalsTab({sessions, hdcp, userId, lang}){
  ...
  return (
    <div>
              ...
              <div key={idx} style={{background:g.reached?GREEN_BG:WHITE,borderRadius:18,padding:18,border:"1.5px solid "+(g.reached?GREEN+"55":(urgent?AMBER+"66":BORDER)),boxShadow:"0 1px 8px rgba(0,0,0,0.04)"}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8,gap:8}}>
                  ...
                  <div style={{textAlign:"center"}}>
                    ...
                    <div style={{color:NAVY,fontSize:22,fontWeight:700,fontFamily:"Georgia,serif"}}>
                ...
                <div style={{background:OFF,borderRadius:8,height:10,overflow:"hidden"}}>
                  <div style={{height:"100%",width:g.progressPct+"%",background:g.reached?GREEN_GRAD:"linear-gradient(90deg,"+PURPLE+",#9d85d4)",borderRadius:8,transition:"width 0.3s"}}/>
                </div>
      ...
      {showHistory&&<GoalHistoryModal archive={archive} lang={lang} onClose={()=>setShowHistory(false)}/>}
...
function GoalProgressModal({goal, metric, sessions, lang, onClose}){
  ...
  const plotW=W-padL-padR, plotH=H-padT-padB;
  const vMin = values.length ? Math.min(...values, goal.target, goal.startValue) : 0;
  const vMax = values.length ? Math.max(...values, goal.target, goal.startValue) : 1;
  const span = vMax-vMin || 1;
  // Oldest round is index 0 -> left edge (padL); newest round is the last
  // index -> right edge (W-padR). Reads chronologically left to right.
  const xAt = i => padL + (roundPoints.length>1 ? (i/(roundPoints.length-1))*plotW : plotW/2);
  const yAt = v => padT + plotH - ((v-vMin)/span)*plotH;
      ...
      <div style={{background:WHITE,borderRadius:"24px 24px 0 0",padding:24,width:"100%",maxWidth:500,maxHeight:"88vh",overflowY:"auto",boxShadow:"0 -8px 40px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:18}}>
          <div style={{background:OFF,borderRadius:12,padding:"10px 8px",textAlign:"center"}}>
            ...
            <div style={{color:TEXT_S,fontSize:15,fontWeight:700,fontFamily:"Georgia,serif"}}>{Math.round(goal.startValue*10)/10}{metric.suffix||""}</div>
          </div>
          <div style={{background:GREEN_BG,borderRadius:12,padding:"10px 8px",textAlign:"center"}}>
            ...
            <div style={{color:NAVY,fontSize:17,fontWeight:700,fontFamily:"Georgia,serif"}}>{current!=null?Math.round(current*10)/10+(metric.suffix||""):"—"}</div>
          </div>
          <div style={{background:PURPLE_BG,borderRadius:12,padding:"10px 8px",textAlign:"center"}}>
            ...
            <div style={{color:NAVY,fontSize:15,fontWeight:700,fontFamily:"Georgia,serif"}}>{goal.target}{metric.suffix||""}</div>
          </div>
        </div>
        ...
        {values.length>1?(
          <div style={{background:OFF,borderRadius:16,padding:14,marginBottom:18}}>
            <svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",height:"auto",display:"block"}}>
              {[0,0.5,1].map(f=>(
                <g key={f}>
                  <line x1={padL} y1={padT+plotH*f} x2={W-padR} y2={padT+plotH*f} stroke={BORDER} strokeWidth="1" strokeDasharray={f===0||f===1?"0":"2 3"} opacity={f===0||f===1?0.6:0.35}/>
                  <text x={padL-6} y={padT+plotH*f+3} fontSize="9" fill={SLATE_L} textAnchor="end">{Math.round((vMax-f*span)*10)/10}{metric.suffix||""}</text>
                </g>
              ))}
              {/* target line, dashed, so it's visually clear what "done" looks like on the same scale as the trend */}
              <line x1={padL} y1={yAt(goal.target)} x2={W-padR} y2={yAt(goal.target)} stroke={GREEN} strokeWidth="1.5" strokeDasharray="3 3" opacity={0.7}/>
              {/* Each point/segment is colored by whether THAT round's value
                  has actually reached or beaten the target — green if so,
                  red if not — independent of whether it moved closer or
                  further away from the previous round. A round that's still
                  short of the target reads red even if it's an improvement
                  on the round before it; one that's already at/past the
                  target reads green even if it dipped slightly from an
                  even-better previous round. Direction-agnostic: works the
                  same whether the metric is lower-is-better (e.g. 3-putts,
                  where AT-OR-BELOW target is the goal) or higher-is-better
                  (e.g. GIR%, where AT-OR-ABOVE target is the goal). */}
              {(()=>{
                const metricLowerIsBetter = metric.lowerIsBetter;
                const hitsTarget = v => metricLowerIsBetter ? v<=goal.target : v>=goal.target;
                return values.slice(1).map((v,idx)=>{
                  const i = idx+1;
                  const prev = values[i-1];
                  const segColor = hitsTarget(v) ? GREEN : RED;
                  return (
                    <line key={i} x1={xAt(i-1)} y1={yAt(prev)} x2={xAt(i)} y2={yAt(v)} stroke={segColor} strokeWidth="2.6" strokeLinecap="round"/>
                  );
                });
              })()}
              {values.map((v,i)=>{
                const hitsTarget = metric.lowerIsBetter ? v<=goal.target : v>=goal.target;
                const dotColor = hitsTarget ? GREEN : RED;
                return <circle key={i} cx={xAt(i)} cy={yAt(v)} r={i===values.length-1?4:2.5} fill={dotColor} stroke={WHITE} strokeWidth="1.2"/>;
              })}
              <text x={padL} y={H-4} fontSize="8.5" fill={SLATE_L}>{fmtChartAxis(roundPoints[0].ts)}</text>
              <text x={W-padR} y={H-4} fontSize="8.5" fill={SLATE_L} textAnchor="end">{fmtChartAxis(roundPoints[roundPoints.length-1].ts)}</text>
            </svg>
            <div style={{display:"flex",alignItems:"center",gap:14,marginTop:8,justifyContent:"center",flexWrap:"wrap"}}>
...
function GoalHistoryModal({archive, lang, onClose}){
        ...
        <div style={{background:WHITE,borderRadius:16,padding:16,marginBottom:16,border:"1px solid "+BORDER,boxShadow:"0 1px 8px rgba(0,0,0,0.04)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            <div style={{background:OFF,borderRadius:12,padding:"12px 14px",textAlign:"center",border:"1px solid "+BORDER}}>
              ...
              <div style={{color:NAVY,fontSize:24,fontWeight:700,fontFamily:"Georgia,serif"}}>{fmtDerivedHcp(hdcpNum)}</div>
            </div>
            <div style={{background:OFF,borderRadius:12,padding:"12px 14px",textAlign:"center",border:"1px solid "+BORDER}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:6}}>
            {[[t(lang,"puttingHcp"),puttingHcp],[t(lang,"shortGameHcp"),shortGameHcp],[t(lang,"longGameHcp"),longGameHcp]].map(([l,v])=>(
              <div key={l} style={{background:PURPLE_BG,borderRadius:16,padding:"12px 8px",textAlign:"center",border:"1px solid "+PURPLE+"33"}}>
                ...
                <div style={{color:NAVY,fontSize:22,fontWeight:700,fontFamily:"Georgia,serif"}}>{fmtDerivedHcp(v)}</div>
              </div>
            ))}
          </div>
...
function seedDemoData(){
  try{
    ...
    function genRound(course, daysAgo, seedBase, feeling, scoreLo, scoreHi, roundType){
      ...
      const finalHoles = holes.map((h,i)=>{
        ...
        if(h.rel<=-1){
          ...
          // Par: GIR + 2 putts most of the time, or missed green needing an
          // up-and-down. FIXED: up-and-down success is no longer guaranteed
          // (was always putts=1, inflating the up-and-down% well above
          // realistic for a 2.0 handicap — real-world target ~45%, the old
          // logic produced ~58%). Now genuinely succeeds only ~55% of the
          // time; the rest is a longer up-and-down (2 putts, still a par via
          // a 3rd approach stroke).
    ...
    // ── 3 example Goals, tied to this player's REAL generated progress —
    ...
    // Goal 1: GIR%, created ~8 months ago (roughly a third of the way through
    // the rounds logged so far at that point), targeting a realistic 5-point
    // improvement from wherever the player's GIR% stood then.
    const goal1CreatedAt = now - 240*DAY;
    const goal1Baseline = roundsAsOf(goal1CreatedAt);
    const goal1Start = computeGoalMetric("girPct", goal1Baseline) ?? 47;
    // Goal 2: 3-Putts per round, created ~5 months ago, targeting a realistic
    // reduction.
    const goal2CreatedAt = now - 150*DAY;
    const goal2Baseline = roundsAsOf(goal2CreatedAt);
    const goal2Start = computeGoalMetric("threePutts18", goal2Baseline) ?? 1.2;
    // Goal 3: Up & Down %, created ~3 months ago, targeting a realistic
    // improvement in scrambling conversion.
    const goal3CreatedAt = now - 90*DAY;
    const goal3Baseline = roundsAsOf(goal3CreatedAt);
    const goal3Start = computeGoalMetric("updownPct", goal3Baseline) ?? 35;
    const goals = [
