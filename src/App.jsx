...
const HANDICAP_BENCHMARKS = [
  {hcp:0,  putts:31.3, girPct:56.5, updownPct:50.0, threePuttPct:3},
  {hcp:5,  putts:32.5, girPct:46.1, updownPct:37.7, threePuttPct:7},
  {hcp:10, putts:33.9, girPct:37.3, updownPct:31.6, threePuttPct:9},
  {hcp:15, putts:34.8, girPct:26.4, updownPct:25.1, threePuttPct:11},
  {hcp:20, putts:36.1, girPct:22.4, updownPct:21.7, threePuttPct:13},
  {hcp:25, putts:37.0, girPct:20.0, updownPct:20.3, threePuttPct:15},
];
...
// 3-putt rate (averaged, since both reflect putting skill), and a "Short
// Game Handicap" from their real up-and-down conversion rate. Returns null
// if there isn't enough data yet (fewer than 3 rounds) rather than showing
// a misleading number from a tiny sample.
function deriveHandicaps(roundCount, avgPutts, threePuttAvg18, updownAvgPct){
  if(roundCount < 3) return {puttingHcp:null, shortGameHcp:null};
  const threePuttPctEquiv = (threePuttAvg18/18)*100; // rough equivalent for benchmark comparison
  const fromPutts = interpolateHandicap(avgPutts, "putts", false);
  const fromThreePutts = interpolateHandicap(threePuttPctEquiv, "threePuttPct", false);
  ...
  return {puttingHcp, shortGameHcp};
}
...
const TR = {
  en: {
    ...
    derivedHandicaps:"Estimated Skill Handicaps", puttingHcp:"Putting Handicap", shortGameHcp:"Short Game Handicap",
    ...
    handicapHistory:"Handicap History", howCalculated:"How this is calculated", yourLatestNumbers:"Your latest numbers (last 5 rounds)",
    hcpCalcExplain:"Putting Handicap averages two figures from your last 5 rounds — your putts per 18 holes and your 3-putt rate — each matched against the benchmark table below. Short Game Handicap comes from your up-and-down conversion rate, matched the same way. The table is built from real Shot Scope and Arccos player data across thousands of rounds.",
  ...
  fr: {
    ...
    derivedHandicaps:"Handicaps de Competence Estimes", puttingHcp:"Handicap de Putting", shortGameHcp:"Handicap de Petit Jeu",
    ...
    handicapHistory:"Historique du Handicap", howCalculated:"Comment c'est calcule", yourLatestNumbers:"Vos derniers chiffres (5 dernieres parties)",
    hcpCalcExplain:"Le Handicap de Putting moyenne deux chiffres de vos 5 dernieres parties — vos putts par 18 trous et votre taux de 3-putts — chacun compare au tableau de reference ci-dessous. Le Handicap de Petit Jeu vient de votre taux de conversion up-and-down, compare de la meme facon. Le tableau est construit a partir de vraies donnees de joueurs Shot Scope et Arccos sur des milliers de parties.",
    hcpLevel:"Handicap", threePuttsShort:"3-Putts", upAndDownShort:"Up&Down", scratch:"Scratch",
  ...
  es: {
    ...
    derivedHandicaps:"Handicaps de Habilidad Estimados", puttingHcp:"Handicap de Putt", shortGameHcp:"Handicap de Juego Corto",
    ...
    handicapHistory:"Historial de Handicap", howCalculated:"Como se calcula", yourLatestNumbers:"Tus ultimos numeros (ultimas 5 rondas)",
    hcpCalcExplain:"El Handicap de Putt promedia dos cifras de tus ultimas 5 rondas — tus putts por 18 hoyos y tu tasa de 3-putts — cada uno comparado con la tabla de referencia abajo. El Handicap de Juego Corto viene de tu tasa de conversion up-and-down, comparada de la misma forma. La tabla esta construida a partir de datos reales de jugadores Shot Scope y Arccos en miles de rondas.",
    hcpLevel:"Handicap", threePuttsShort:"3-Putts", upAndDownShort:"Up&Down", scratch:"Scratch",
  ...
  de: {
    ...
    derivedHandicaps:"Geschaetzte Skill-Handicaps", puttingHcp:"Putting-Handicap", shortGameHcp:"Kurzspiel-Handicap",
    ...
    handicapHistory:"Handicap-Verlauf", howCalculated:"So wird es berechnet", yourLatestNumbers:"Deine neuesten Zahlen (letzte 5 Runden)",
    hcpCalcExplain:"Das Putting-Handicap mittelt zwei Werte aus deinen letzten 5 Runden — deine Putts pro 18 Loecher und deine 3-Putt-Rate — jeweils mit der Vergleichstabelle unten abgeglichen. Das Kurzspiel-Handicap kommt aus deiner Up-and-Down-Erfolgsrate, genauso abgeglichen. Die Tabelle basiert auf echten Shot Scope und Arccos Spielerdaten aus Tausenden von Runden.",
    hcpLevel:"Handicap", threePuttsShort:"3-Putts", upAndDownShort:"Up&Down", scratch:"Scratch",
  ...
  it: {
    ...
    derivedHandicaps:"Handicap di Abilita Stimati", puttingHcp:"Handicap di Putting", shortGameHcp:"Handicap di Gioco Corto",
  ...
  pt: {
    ...
    derivedHandicaps:"Handicaps de Habilidade Estimados", puttingHcp:"Handicap de Putting", shortGameHcp:"Handicap de Jogo Curto",
    ...
    handicapHistory:"Historico de Handicap", howCalculated:"Como isso e calculado", yourLatestNumbers:"Seus numeros mais recentes (ultimas 5 rodadas)",
    hcpCalcExplain:"O Handicap de Putting faz a media de dois numeros das suas ultimas 5 rodadas — seus putts por 18 buracos e sua taxa de 3-putts — cada um comparado com a tabela de referencia abaixo. O Handicap de Jogo Curto vem da sua taxa de conversao up-and-down, comparada da mesma forma. A tabela e construida a partir de dados reais de jogadores Shot Scope e Arccos em milhares de rodadas.",
    hcpLevel:"Handicap", threePuttsShort:"3-Putts", upAndDownShort:"Up&Down", scratch:"Scratch",
...
function HandicapHistoryDetail({rounds, lang, onBack}){
  ...
  // down to the minimum of 3 deriveHandicaps itself requires). This is what
  // makes "history" meaningful: each point reflects what the handicap
  // genuinely was at that moment, not a single all-time figure repeated.
  const snapshots = roundsChrono.map((r,i)=>{
    ...
    const {puttingHcp, shortGameHcp} = deriveHandicaps(windowRounds.length, avgPutts, threePuttAvg18, updownPct);
    return {ts:r.ts, puttingHcp, shortGameHcp, avgPutts, threePuttAvg18, updownPct};
  });

  const ranged = range==="all" ? roundsChrono : roundsChrono.slice(-parseInt(range,10));
  const puttingTrend = snapshots.filter(s=>s.puttingHcp!=null).slice(range==="all"?0:-parseInt(range,10)).map(s=>s.puttingHcp);
  const shortGameTrend = snapshots.filter(s=>s.shortGameHcp!=null).slice(range==="all"?0:-parseInt(range,10)).map(s=>s.shortGameHcp);
  ...
  const rangedUpdownAttempts = rangedHoles.filter(h=>h.gir===false);
  const rangedUpdownSuccess = rangedUpdownAttempts.filter(h=>(h.putts||0)<=1);
  const rangedUpdownPct = rangedUpdownAttempts.length ? (rangedUpdownSuccess.length/rangedUpdownAttempts.length)*100 : null;
  const latest = {
    ...deriveHandicaps(ranged.length, rangedAvgPutts, rangedThreePuttAvg18, rangedUpdownPct),
    avgPutts: rangedAvgPutts, threePuttAvg18: rangedThreePuttAvg18, updownPct: rangedUpdownPct,
  };
      ...
      {snapshots.length===0 ? (
        ...
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
            {[[t(lang,"puttingHcp"),latest?.puttingHcp],[t(lang,"shortGameHcp"),latest?.shortGameHcp]].map(([l,v])=>(
              <div key={l} style={{background:PURPLE_BG,borderRadius:14,padding:"12px 8px",textAlign:"center",border:"1px solid "+PURPLE+"33"}}>
                <div style={{color:PURPLE,fontSize:9,textTransform:"uppercase",letterSpacing:0.5,fontWeight:700,marginBottom:2}}>{l}</div>
                <div style={{color:NAVY,fontSize:19,fontWeight:700,fontFamily:"Georgia,serif"}}>{v!=null?v:"—"}</div>
              </div>
            ))}
          </div>

          {puttingTrend.length>1&&(
            <div style={{background:WHITE,borderRadius:16,padding:16,marginBottom:12,border:"1px solid "+BORDER,boxShadow:"0 1px 8px rgba(0,0,0,0.04)"}}>
              <div style={{color:NAVY,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>{t(lang,"puttingHcp")}</div>
              <div style={{color:SLATE_L,fontSize:10,marginBottom:10}}>{t(lang,"downMeansImproving")}</div>
              <Sparkline data={puttingTrend} color={PURPLE} invert/>
            </div>
          )}
          {shortGameTrend.length>1&&(
            <div style={{background:WHITE,borderRadius:16,padding:16,marginBottom:18,border:"1px solid "+BORDER,boxShadow:"0 1px 8px rgba(0,0,0,0.04)"}}>
              <div style={{color:NAVY,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>{t(lang,"shortGameHcp")}</div>
              <div style={{color:SLATE_L,fontSize:10,marginBottom:10}}>{t(lang,"downMeansImproving")}</div>
              <Sparkline data={shortGameTrend} color={PURPLE} invert/>
            </div>
          )}
            ...
            {latest&&latest.avgPutts!=null&&(
              <div style={{background:OFF,borderRadius:10,padding:"10px 12px",marginBottom:10}}>
                ...
                <div style={{color:TEXT,fontSize:12,lineHeight:1.7}}>
                  ...
                  {latest.updownPct!=null&&<>{t(lang,"upAndDown")}: <b>{Math.round(latest.updownPct*10)/10}%</b></>}
                </div>
              </div>
            )}
            <div style={{overflowX:"auto"}}>
                ...
                <tbody>
                  {HANDICAP_BENCHMARKS.map((b,i)=>(
                    <tr key={b.hcp} style={{borderBottom:i<HANDICAP_BENCHMARKS.length-1?"1px solid "+BORDER:"none"}}>
                      ...
                      <td style={{textAlign:"right",padding:"6px 4px",color:TEXT_S}}>{b.threePuttPct}%</td>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
            {[[t(lang,"avgScore"),avgScore!=null?avgScore+"/"+(game?.max||""):"—"],[t(lang,"best"),bestScore!=null?bestScore+"/"+(game?.max||""):"—"]].map(([l,v])=>(
              <div key={l} style={{background:"#fff8e6",borderRadius:14,padding:"12px 8px",textAlign:"center",border:"1px solid "+GOLD+"55"}}>
                ...
                <div style={{color:NAVY,fontSize:19,fontWeight:700,fontFamily:"Georgia,serif"}}>{v}</div>
              </div>
            ))}
          </div>
...
function StatsTab({sessions, lang, hdcp, onGoTrain, onGoScore}){
  ...
  const allUpdownAttempts = allH.filter(h=>h.gir===false);
  const allUpdownSuccess = allUpdownAttempts.filter(h=>(h.putts||0)<=1);
  const updownConversionPct = allUpdownAttempts.length
    ? Math.round((allUpdownSuccess.length/allUpdownAttempts.length)*1000)/10
    : null;
  const {puttingHcp, shortGameHcp} = deriveHandicaps(rounds.length, avgP, threePuttAvg, updownConversionPct);
  ...
  return (
    <div>
      ...
      {(puttingHcp!=null||shortGameHcp!=null)&&(
        <div style={{marginBottom:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6}}>
            {[[t(lang,"puttingHcp"),puttingHcp],[t(lang,"shortGameHcp"),shortGameHcp]].map(([l,v])=>(
              <div key={l} style={{background:PURPLE_BG,borderRadius:16,padding:"14px 16px",textAlign:"center",border:"1px solid "+PURPLE+"33"}}>
                <div style={{color:PURPLE,fontSize:10,textTransform:"uppercase",letterSpacing:1,fontWeight:700,marginBottom:2}}>{l}</div>
                <div style={{color:NAVY,fontSize:28,fontWeight:700,fontFamily:"Georgia,serif"}}>{v!=null?v:"—"}</div>
              </div>
            ))}
          </div>
