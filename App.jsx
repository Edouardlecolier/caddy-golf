...
function ScorecardEntry({holeCount, onSave, onCancel, lang, userId}){
  ...
  function applyCourse(c){
    ...
    setHoles(prev=>prev.map((h,i)=>({...h, par: courseHole(c, i)})));
  }
...
function seedDemoData(){
  try{
    ...
    const user = {
      ...
      hdcp:"14.2", freq:"f23", photo:"", goal:"break80", weaknesses:["bunker","mental"],
      homeClub:"", location:"", typicalScore:"85_89", hand:"Right"
    };
    ...
    // ── 30 training sessions across all 4 modules, oldest -> newest, with a
    // gentle upward trend so the demo tells a believable "improving" story.
    const picks = [
      ["p1","putting","Gate Drill","🚪","Beginner",20,"Improves start line and face control."],
      ["p3","putting","Straight Putt","📏","Beginner",15,"Stroke mechanics and tempo"],
      ["p4","putting","Lag to the Fringe","🎯","Beginner",10,"Distance control and speed"],
      ["p6","putting","Ladder to the Hole","🪜","Beginner",12,"Basic speed judgment"],
      ["p7","putting","Clock Drill","🕐","Intermediate",12,"Consistency from all angles"],
      ["p8","putting","Distance Ladder","📶","Intermediate",15,"Speed calibration"],
      ["p9","putting","5-Footer Pressure","💪","Intermediate",20,"Pressure performance"],
      ["p10","putting","Breaking Putt","↩️","Intermediate",10,"Break reading"],
      ["p12","putting","100-Ball Putting","💯","Advanced",100,"Endurance and baseline"],
      ["s1","shortGame","Up and Down","⛳","Beginner",10,"Basic chipping and finishing"],
      ["s2","shortGame","Landing Zone","🎯","Beginner",15,"Landing spot awareness"],
      ["s3","shortGame","Chip to a Tee","📌","Beginner",10,"Precision proximity control"],
      ["s5","shortGame","Bump and Run","🎳","Intermediate",10,"Low trajectory shot control"],
      ["s6","shortGame","Pick Your Club","🏑","Intermediate",15,"Club selection confidence"],
      ["s7","shortGame","Bunker Escape","🏖️","Intermediate",10,"Sand technique"],
      ["s8","shortGame","Pitch Precision","📡","Intermediate",10,"Wedge distance control"],
      ["l1","longGame","Alignment Drill","📏","Beginner",20,"Setup and alignment"],
      ["l2","longGame","Slow Motion Swings","🐢","Beginner",15,"Swing plane and sequencing"],
      ["l3","longGame","Feet Together","🦵","Beginner",15,"Balance and rhythm"],
      ["l4","longGame","Low Point Control","🎯","Beginner",15,"Ball-then-turf contact"],
      ["l5","longGame","Fairway Finder","🛣️","Intermediate",10,"Driver accuracy"],
      ["l6","longGame","Iron Accuracy at 130m","🎯","Intermediate",15,"Mid-range iron dispersion"],
      ["l8","longGame","Wedge Distance Ladder","📶","Intermediate",15,"Partial swing calibration"],
      ["l9","longGame","Stock Shot Repeatability","🔁","Intermediate",15,"Consistency under no pressure"],
      ["m1","mental","Pre-Shot Routine Builder","🧘","Beginner",15,"Building a repeatable routine"],
      ["m2","mental","Visualization Reps","💭","Beginner",10,"Mental imagery before contact"],
      ["m3","mental","One Breath Reset","🌬️","Beginner",15,"Resetting after a bad shot"],
      ["m4","mental","Commitment Drill","🎯","Intermediate",10,"Trusting your read or plan"],
      ["m5","mental","Pressure Putt Simulation","💓","Intermediate",10,"Performing under simulated pressure"],
      ["m9","mental","Process Over Outcome","⚖️","Advanced",15,"Detaching from results"],
    ];

    const trainSessions = picks.map((p, i) => {
      const [exId, focusId, exName, exEmoji, exDiff, exMax, exPurpose] = p;
      // Gentle improvement curve: earliest sessions ~55-70%, latest ~75-92%.
      const progress = i / (picks.length - 1); // 0 -> 1
      const basePct = 0.55 + progress * 0.32;
      const noise = (Math.sin(i * 7.3) * 0.07); // small deterministic wobble, not random each load
      const pct = Math.max(0.4, Math.min(0.96, basePct + noise));
      const score = Math.max(1, Math.round(exMax * pct));
      // Spread across the last 26 days, oldest first.
      const daysAgo = 26 - Math.round((i / (picks.length - 1)) * 25);
      const ts = now - daysAgo*DAY - (i*37*60000); // small time-of-day offset per session
      return {type:"train", focusId, exId, exName, exEmoji, exDiff, exPurpose, exMax, exLabel:"Score", score, ts};
    });

    // ── Finisher games: Edouard vs Alban L and Simon M, local multiplayer,
    // three separate game sessions (Putting, Short Game, Long Game), each
    // producing one record per player. Edouard is marked isAccount:true.
    function finisherTrio(gameId, gameName, max, stationIds, daysAgo, edouardPct, albanPct, simonPct){
      const baseTs = now - daysAgo*DAY;
      const sessionId = "fs_demo_"+gameId; // shared by all 3 players in this one playthrough
      let idx = 0;
      const mk = (playerName, pct, isAccount) => {
        const score = Math.round(max*pct);
        // distribute the score across stations proportionally for a believable breakdown
        ...
        const rec = {type:"finisher", sessionId, gameId, gameName, playerName, score, max, stationScores, multiplayer:true, isAccount, ts:baseTs+idx};
        idx++;
        return rec;
      };
      return [
        mk("Edouard L", edouardPct, true),
        mk("Alban L", albanPct, false),
        mk("Simon M", simonPct, false),
      ];
    }

    const finisherSessions = [
      ...finisherTrio("putting","Putting Finisher",100,["pt1","pt2","pt3","pt4","pt5"], 18, 0.74, 0.68, 0.81),
      ...finisherTrio("shortGame","Short Game Finisher",100,["sg1","sg2","sg3","sg4","sg5"], 11, 0.62, 0.7, 0.59),
      ...finisherTrio("longGame","Long Game Finisher",100,["lg1","lg2","lg3","lg4"], 4, 0.71, 0.64, 0.77),
    ];

    // ── 7 scorecards using real courses from COURSES, realistic for a ~14
    // handicap (mostly bogey golf with some pars, occasional birdie/double).
    // Deterministic "random" via a seeded pattern so resetting the demo
    // always reproduces the same believable rounds, not different ones each time.
    function pseudoRand(seed){ const x = Math.sin(seed*999)*10000; return x - Math.floor(x); }
    function genRound(courseName, daysAgo, holeCount, feeling){
      const c = COURSES.find(x=>x.name===courseName) || COURSES.find(x=>x.id==="g18");
      const pars = c.holes.slice(0, holeCount);
      const holes = pars.map((par,i)=>{
        const r = pseudoRand(daysAgo*100+i);
        // Score relative to par: weighted toward bogey (+1), some pars, rare birdie/double.
        let rel;
        if(r<0.08) rel=-1;       // birdie
        else if(r<0.40) rel=0;   // par
        else if(r<0.85) rel=1;   // bogey
        else rel=2;              // double
        const score = Math.max(1, par+rel);
        const putts = par<=3 ? (rel<=0?2:Math.min(3,2+Math.round(r))) : (rel<=0?2:(r>0.7?3:2));
        const gir = rel<=0;
        const fir = par>3 ? r>0.35 : null;
        const missOptions=["left","right","short","long"];
        const miss = (!gir && r>0.5) ? missOptions[Math.floor(r*4)%4] : "";
        return {hole:i+1, par, score, putts, fir: par>3?(fir===true):false, gir, miss};
      });
      return {type:"round", holes, n:holeCount, course:c.name, feeling, notes:"", ts: now - daysAgo*DAY};
    }

    const roundSessions = [
      genRound("Pebble Beach Golf Links", 24, 18, "focused"),
      genRound("St Andrews Old Course", 19, 18, "relaxed"),
      genRound("Le Golf National", 15, 18, "neutral"),
      genRound("Valderrama Golf Club", 9, 9, "tired"),
      genRound("Royal County Down", 7, 18, "pumped"),
      genRound("Carnoustie Golf Links", 3, 18, "focused"),
      genRound("TPC Sawgrass (Stadium Course)", 1, 9, "anxious"),
    ];

    const allSessions = [...trainSessions, ...finisherSessions, ...roundSessions].sort((a,b)=>b.ts-a.ts); // newest-first, matches addSession()
