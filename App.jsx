...
const HANDICAP_BENCHMARKS = [
  {hcp:0,  putts:31.3, girPct:56.5, updownPct:50.0, threePuttsPerRound:0.6},
  {hcp:5,  putts:32.5, girPct:46.1, updownPct:37.7, threePuttsPerRound:0.9},
  {hcp:10, putts:33.9, girPct:37.3, updownPct:31.6, threePuttsPerRound:1.7},
  {hcp:15, putts:34.8, girPct:26.4, updownPct:25.1, threePuttsPerRound:2.6},
  {hcp:20, putts:36.1, girPct:22.4, updownPct:21.7, threePuttsPerRound:3.3},
  {hcp:25, putts:37.0, girPct:20.0, updownPct:20.3, threePuttsPerRound:4.1},
];
        ...
        <>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:18}}>
            {[...ranged].sort((a,b)=>b.ts-a.ts).map(r=>{
              ...
              const updownAttempts=holes.filter(h=>h.gir===false);
              const updownSuccess=updownAttempts.filter(h=>(h.putts||0)<=1);
              const updownStr = updownAttempts.length ? updownSuccess.length+"/"+updownAttempts.length : "—";
              return (
                <div key={r.ts} onClick={()=>setModal(r)} style={{background:"linear-gradient(135deg,#f4f0fa,#f8f5fc)",borderRadius:14,padding:"12px 14px",border:"1.5px solid "+PURPLE+"33",cursor:"pointer"}}>
                  ...
                  <div style={{display:"flex",gap:16,paddingTop:6,borderTop:"1px solid "+PURPLE+"22"}}>
